// Multi-thread T-MAC NEON benchmark with correctness hardening.
// The production packing/LUT/NEON compute path remains frozen.
// Verification adds frozen-ST differential checks, guarded output coverage, and partition-boundary cases.
...
inline MtOptions parse_mt_options(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (arg == "--bits" && i + 1 < argc) options.bits = std::stoi(argv[++i]);
            ...
            "Usage: program [--bits 2|3|4] [--threads N] [--max-size 256|1024|2048|4096|8192] [--verify-only]");
    }
    if (options.bits < 2 || options.bits > 4) throw std::invalid_argument("--bits must be 2, 3, or 4");
    if (options.threads <= 0 || options.threads > 16) throw std::invalid_argument("--threads must be between 1 and 16");
    return options;
}
...
inline void print_mt_benchmark_header(int bits, int threads)
{
    std::cout << '\n';
    bench::print_separator();
    std::cout << "TMAC_NEON_MT W" << bits << "A16 Benchmark | Threads=" << threads << '\n';
    std::cout << "Matrix: W[MxK], X[NxK], Y[NxM] = X * W^T\n";
    std::cout << "Official parallel rule: N/BN >= threads => N tiles; otherwise M tiles.\n";
    std::cout << "Timing includes one single-thread activation-LUT construction plus parallel compute; offline packing and thread-pool construction are excluded.\n";
    bench::print_separator('-');
...
template <int Bits>
void run_shape(const bench::Shape& shape, Runtime& runtime)
{
    ...
    // The legal candidate space is exactly Microsoft's. Candidate selection remains a standalone benchmark policy,
    // because the official framework obtains the final BM/BN from target-specific AutoTVM tuning.
    ...
    {
        tmac_neon_mt<Bits>(packed_weights, packed_scales, shape.n, activations.data(), output.data(), runtime);
    });

    const ParallelAxis axis = official_parallel_axis(shape.n, schedule.bn, runtime.pool.num_threads());
    int parallel_tiles = 1;
    if (axis == ParallelAxis::N) parallel_tiles = (shape.n + schedule.bn - 1) / schedule.bn;
    else if (axis == ParallelAxis::M) parallel_tiles = shape.m * Bits / schedule.bm;

    std::cout << std::left << std::setw(7) << shape.kind() << std::right << std::setw(7) << shape.m << std::setw(7) << shape.k
              << std::setw(7) << shape.n << std::setw(7) << schedule.bm << std::setw(7) << schedule.bn << std::setw(6) << schedule.kfactor
              << std::setw(8) << parallel_axis_name(axis) << std::setw(8) << parallel_tiles << std::setw(7) << shape.warmup
              << std::setw(7) << shape.repeat << std::setw(8) << shape.samples << std::fixed << std::setprecision(4)
              << std::setw(14) << stats.median_ms << std::setw(12) << stats.p90_ms
              << std::setw(12) << bench::dense_equivalent_gops(shape, stats.median_ms)
              << std::scientific << std::setprecision(4) << std::setw(15) << bench::checksum(output) << std::fixed << '\n';
}
...
int run(const MtOptions& options)
{
    ...
    print_mt_benchmark_header(Bits, options.threads);
    for (const bench::Shape& shape : bench::benchmark_shapes())
        if (shape.m <= options.max_size) run_shape<Bits>(shape, runtime);
