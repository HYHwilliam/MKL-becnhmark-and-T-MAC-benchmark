...
// ==== lut_ctor / partial_max：與先前驗證通過版本完全相同 ====
...
// ==== 多執行緒版：接受 start_m/end_m，只計算指定區間 ====
// 注意：qlut/scales/lut_scales/lut_biases 是全域共享的唯讀資料，所有 thread 讀同一份
// 只有 m 維度（c 的輸出範圍、a 的權重範圍）依 start_m/end_m 切分
template <int Bits, int ActK=32>
inline void tbl_update_avx2_mt(int32_t start_m,int32_t end_m,int32_t k_groups,float* c,int8_t* lut,uint8_t* a,
                               float* scales,float* lut_scales,float* lut_biases) {
    const __m128i vec_mask=_mm_set1_epi8(0x0f);
    __m128i* vec_lut=(__m128i*)_mm_malloc(k_groups*sizeof(__m128i),32);
    for (int k=0;k<k_groups;k++) vec_lut[k]=_mm_loadu_si128(reinterpret_cast<__m128i*>(lut+k*16));
    ...
    // i 是以 "整體 m" 為基準的位移，start_m/end_m 必須是 32 的倍數才能對齊 i 的步進(16)
    for (int i=start_m/2; i<end_m/2; i+=16) {
        __m256 c0=_mm256_setzero_ps(),c1=_mm256_setzero_ps(),c2=_mm256_setzero_ps(),c3=_mm256_setzero_ps();
        for (int kk=0;kk<k_groups;kk+=ActK) {
            for (int k=0;k<ActK;k++) {
                // a 的位址仍以 "整體 i" 計算，因為 weights 陣列本身是按整體 m 佈局，只是這個 thread 只碰它負責的區段
                __m128i as=_mm_loadu_si128(reinterpret_cast<const __m128i*>(a+i*k_groups+(kk+k)*16));
                __m128i bot=_mm_and_si128(as,vec_mask), top=_mm_and_si128(_mm_srli_epi16(as,4),vec_mask);
                __m256i lut_=_mm256_set_m128i(vec_lut[kk+k],vec_lut[kk+k]);
                __m256i av=_mm256_set_m128i(top,bot);
...
struct ThreadArgs {
    int32_t start_m, end_m, k_groups;
    float* out_c;
    int8_t* qlut;
    uint8_t* weights;
    float* scales;
...
void* worker_thread(void* arg) {
    ...
    tbl_update_avx2_mt<4, 32>(a->start_m, a->end_m, a->k_groups,
                              a->out_c, a->qlut, a->weights,
                              a->scales, a->lut_scales, a->lut_biases);
    return nullptr;
}

// 將 m 切成 num_threads 份，每份對齊到 32 的倍數，確保不會切在非法邊界
void compute_chunks(int m, int num_threads, std::vector<int>& starts, std::vector<int>& ends) {
    int base_chunk = (m / num_threads / 32) * 32; // 先算基本塊數，再對齊 32
    if (base_chunk == 0) base_chunk = 32;         // 至少給 32，避免 num_threads 過多導致 chunk=0
    starts.resize(num_threads);
    ends.resize(num_threads);
    int cur = 0;
    for (int t = 0; t < num_threads; ++t) {
        starts[t] = cur;
        if (t == num_threads - 1) ends[t] = m; // 最後一個 thread 吃掉所有剩餘
...
int main() {
    ...
    std::cout << "T-MAC Pthread 多執行緒 Benchmark\n";
    std::cout << "------------------------------------------------------\n";
    ...
    for (int size : sizes) {
        ...
        float* out_c_single = (float*)_mm_malloc(m*sizeof(float),32); // 單執行緒對照
        ...
        for (int nt : thread_counts) {
            if (m % (32 * nt) != 0 && nt > 1) {
                // 允許不整除，因為 compute_chunks 會對齊處理，僅提示
            }
            ...
            for (int it = 0; it < iterations; ++it) {
                std::memset(out_c, 0, m*sizeof(float));
                for (int t = 0; t < nt; ++t) {
                    args[t] = {starts[t], ends[t], k_groups, out_c, qlut, weights, scales, lut_scales, lut_biases};
            ...
            // 正確性比對：多執行緒結果應與單執行緒完全一致（同一份 qlut/weights，只是切分運算）
            int mismatches = 0;
            double max_err = 0.0;
            for (int i = 0; i < m; ++i) {
