...
1. `tmac_avx2_gemm_verify.cpp`：以直接量化矩陣乘法 Ground Truth 驗證 packed weight、LUT、bit-plane reduction、schedule candidate、執行緒一致性及 optimized/reference kernel。
2. `tmac_avx2_gemm_w234.cpp`、`mkl_fp16_gemm_mt.cpp` 與 `compare_tmac_mkl_mt.py`：比較 W2A16、W3A16、W4A16 與 oneMKL W16A16 在 1、2、4、8 thread budget 下的效能。
...
| `tmac_avx2_gemm_verify.cpp` | 獨立數學驗證 | T-MAC 對直接量化矩陣乘法 Ground Truth，並驗證 packing、schedule 與 thread determinism |
| `tmac_avx2_gemm_w234.cpp` | T-MAC 效能測試 | W2A16、W3A16、W4A16，官方候選 schedule、offline autotune、median latency 與 P90 |
...
`tmac_avx2_gemm_verify.cpp` 使用同一組量化權重與 FP16 Activation 建立兩條獨立路徑：
