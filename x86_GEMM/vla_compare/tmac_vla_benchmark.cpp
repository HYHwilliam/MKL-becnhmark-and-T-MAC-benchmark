...
std::vector<int> parse_threads(const std::string& text)
{
    ...
    if (result.empty()) result = {1, 2, 4, 8, 16};
    return result;
}
...
void run_suite(const std::vector<int>& thread_counts, bool quick, bool autotune)
{
    const std::vector<Shape> shapes = {
        {2560, 2560, 8,   "BitVLA Action Residual"},
        {2560, 17920, 8,  "BitVLA Action FC1"},
        {1152, 1152, 256, "BitVLA Vision Attention"},

        {2048, 1024, 32,  "Pi0 Expert Q"},
        {256,  1024, 32,  "Pi0 Expert KV"},
        {1024, 2048, 32,  "Pi0 Expert O"},
        {4096, 1024, 32,  "Pi0 Expert GateUp"},
        {1024, 4096, 32,  "Pi0 Expert Down"}
    };

    std::cout << "==================================================================\n";
    std::cout << "T-MAC W" << Bits << "A16 HackMD Benchmark\n";
    std::cout << "FP16 Activation, AVX2/F16C/OpenMP\n";
    std::cout << "==================================================================\n";

    for (const Shape& shape : shapes)
    {
        run_shape<Bits>(shape, thread_counts, autotune);
    }
}
